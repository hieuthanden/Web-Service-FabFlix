...
import edu.uci.ics.hieutt1.service.billing.model.DeleteRequestModel;
...
public class DeleteEndPoint {
    ...
    public Response DeleteItemCart(@Context HttpHeaders headers, String jsonText) {
        ...
        ServiceLogger.LOGGER.warning("Deleting item in the cart....");
        ...
        try {
            ...
            ServiceLogger.LOGGER.warning("Query failed: Unable to delete item from cart");
            e.printStackTrace();
        }
        responseModel = new InsertResponseModel(3120, "Shopping cart item deleted successfully.");
        ServiceLogger.LOGGER.warning("Shopping cart item deleted successfully.");
        Response.ResponseBuilder builder = Response.status(Response.Status.OK).entity(responseModel);
        builder.header("email", email_header);
        builder.header("session_id", session_id);
        builder.header("transaction_id", transaction_id);
